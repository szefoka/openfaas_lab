#!/bin/bash
DASH_ADDR=$(ip addr sh dev $(ip ro sh | grep default | awk '{print $5}') scope global | grep inet | awk '{split($2,addresses,"/"); print addresses[1]}'):$(kubectl get services -n kubernetes-dashboard | grep kubernetes-dashboard | awk '{ print $5 }' | cut -d ':' -f 2 | cut -d '/' -f 1)
